...
class DL(Utils):    
    ...
    def Model(self):
        ...
        OpData = Input(shape=(self.params['xX'],self.params['yY'],2))
        inOP = OpData
        ## Input Multi-Dimensional Fluorescence ##
        FlData = Input(shape=(self.params['xX'],self.params['yY'],self.params['nF'],1))
        inFL = FlData
        ...
        outOP3 = inOP
        #inOP = BatchNormalization()(inOP)
        inOP = self.resblock_2D(self.params['nFilters2D']/2, self.params['kernelResBlock2D'], self.params['strideConv2D'], inOP)
        #inOP = BatchNormalization()(inOP)

        ## Fluorescence Input Branch ##
        input_shape = inFL.shape
        ...
        outFL3 = inFL
        #inFL = BatchNormalization()(inFL)
        inFL = self.resblock_2D(self.params['nFilters2D']/2, self.params['kernelResBlock2D'], self.params['strideConv2D'], inFL)
        #inFL = BatchNormalization()(inFL)
        ...
        self.modelD = Model(inputs=[OpData,FlData], outputs=[outQF, outDF])#,outFL])
    ...
    def Analysis(self):
        ...
        plt1.scatter(self.DF[indxIncl],DF_P[indxIncl],s=1)
        plt1.set_xlim([0, 15])
        plt1.set_ylim([0, 15])
        ...
        plt2.scatter(self.QF[indxIncl],QF_P[indxIncl],s=1)
        plt2.set_xlim([0, 10])
        plt2.set_ylim([0, 10])
        ...
        ## Error Histograms
        fig, (plt1, plt2) = plt.subplots(1, 2)
        plt1.hist(DF_error[indxIncl],bins=100)
        plt2.hist(QF_error[indxIncl],bins=100)
